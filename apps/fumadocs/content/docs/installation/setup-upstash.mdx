...
Vercord uses Upstash Redis for rate limiting. Add the credentials and rate limiting is enabled. No credentials = no rate limiting.

<Callout type="warn">
  Rate limiting is recommended for production to protect against abuse.
</Callout>
